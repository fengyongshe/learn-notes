...
public class HdfsUserManager extends AbstractUserManager {
	...
	/**
	 * Save user data. Store the properties.此处没用到
	 */
	 ...
	 * removing the corresponding from the properties, save the data.应该也没用到
	 */
	...
	/**
	 * Get user password. Returns the encrypted value.没用到
	 * <p/>
	 * 
	 * <pre>
	...
	private synchronized ArrayList<String> parseGroups(String groupsLine) {
		String groupsArray[] = groupsLine.split(",");
		return new ArrayList(Arrays.asList(groupsArray));
	}

	/**
	 * getUserByName的重载方法 Load user data.
	 * 
	 * @param userName
	 * @param pwd
	 * @return
	 */
	public synchronized User getUserByName(String userName, String pwd) {
		HdfsUser user = null;
		FtpElement ele = MyUserManager.getUserManager().getUserElement(
				userName, pwd);
		if (null == ele) {
			return user;
		}
		user = new HdfsUser();
		user.setName(ele.getUserName());
		user.setEnabled(ele.getEnableflag() == 1 ? true : false);
		user.setHomeDirectory(ele.getHomedirectory());
		user.setGroups(parseGroups(ele.getGroups()));
		List<Authority> authorities = new ArrayList<Authority>();
		if (1 == ele.getWritepermission()) {
			authorities.add(new WritePermission());
		}
		int maxLogin = ele.getMaxloginnumber();
		int maxLoginPerIP = ele.getMaxloginperip();
		authorities.add(new ConcurrentLoginPermission(maxLogin, maxLoginPerIP));
		int uploadRate = ele.getUploadrate();
		int downloadRate = ele.getDownloadrate();
		authorities.add(new TransferRatePermission(downloadRate, uploadRate));
		/*
		 * 此处不知道啥意思，可能有问题
		 */
		user.setAuthorities(authorities.toArray(new Authority[0]));
		user.setMaxIdleTime(ele.getIdletime());
		return user;
	}

	/**
	 * Load user data.下面验证账户用到(注释掉，server中会调用到，没用)
	 */
	public User getUserByName(String userName) {
		 ...
		 * 
		 * String baseKey = PREFIX + userName + '.'; HdfsUser user = new
		 * HdfsUser(); user.setName(userName);
		 * user.setEnabled(userDataProp.getBoolean(baseKey + ATTR_ENABLE,
		 * true)); user.setHomeDirectory(userDataProp .getProperty(baseKey +
		 * ATTR_HOME, "/"));
		 * user.setGroups(parseGroups(userDataProp.getProperty(baseKey +
		 * "groups")));
		 * 
		 * List<Authority> authorities = new ArrayList<Authority>();
		 * 
		 ...
		 * ATTR_MAX_LOGIN_NUMBER, 0); int maxLoginPerIP =
		 * userDataProp.getInteger(baseKey + ATTR_MAX_LOGIN_PER_IP, 0);
		 * 
		 * authorities.add(new ConcurrentLoginPermission(maxLogin,
		 * maxLoginPerIP));
		 * 
		 * int uploadRate = userDataProp.getInteger( baseKey +
		 * ATTR_MAX_UPLOAD_RATE, 0); int downloadRate =
		 * userDataProp.getInteger(baseKey + ATTR_MAX_DOWNLOAD_RATE, 0);
		 * 
	...
	public synchronized boolean doesExist(String name) {
		// lazyInit();
		/*
		 * String key = PREFIX + name + '.' + ATTR_HOME; return
		 * userDataProp.containsKey(key);
		 */
		return MyUserManager.getUserManager().isExist(name);
	}
	...
	public synchronized User authenticate(Authentication authentication)
		...
		if (authentication instanceof UsernamePasswordAuthentication) {
			...
			User myUser = getUserByName(user, password);
			if (null != myUser) {
